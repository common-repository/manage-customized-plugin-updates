...
function wsx_mcpu_custom_register_widgets()
{
  ...
  wp_add_dashboard_widget('wsx_mcpu_custom_plugins', __(' Customized Plugin(s) List', 'wsx_rss_feeds') , 'wsx_mcpu_custom_plugins_box');
}
...
function wsx_mcpu_custom_plugins_box()
{
  ...
  if (count($plugins_marked_entries) > 0)
  {
    echo '<table class="mcpu-head-wrap" cellpadding="0" cellspacing="0">
    <tr><th> #</th><th class="wsx-mcpu-pl-name">Plugin Name</th>
    <th class="wsx-mcpu-pl-details">Customization Details</th></tr>';
    ...
    foreach($plugins_marked_entries as $marked_entries)
    {
      ...
      $customization_details = $marked_entries->customization_details;
        ...
        echo '<tr class="wsx-mcpu-admin-list">';
        echo '<td>'.$mcpu_count.'.</td>';
        echo '<td>'.$marked_plugin_name.'</td>';
        
        echo '<td><a class="mcpu-cpn-plugin" href="#">View Details</a>';
        ...
        { 
          ...
          echo '<div class="mcpu-plugin-desc"><h3 class="txt-center">Customization Details</h3><p>No description available.</p></div></td>';
        }

      $mcpu_count++;
    }
    echo '</tbody></table>';
    $wsx_mcpu_plugin_settings_url = esc_url(get_admin_url(null, 'plugins.php'));
    echo '<a class="wsx_mcpu_read_more" href="'.$wsx_mcpu_plugin_settings_url.'">' . __('Go to Plugins', 'manage-customized-plugin-updates') . '</a>';
  } else {
